...
import time # to execute polling every 5 seconds
...
B_ADDR = (IP, 7531)# backup_server port to connect to
SIZE = 1024
FORMAT = "utf-8"
clients = {}  # global list of clients to get updates everytime a new client is pushed into it.
SERVER = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
SERVER.bind(B_ADDR) # Binding the socket with a IP and PORT number.
...
def receive(CLIENT):
    ...
    user_name = "backup_server"
    while True:
        try:
            data = CLIENT.recv(SIZE).decode('utf8')
            words_to_be_added_list = json.loads(data)["list"]
            for word in words_to_be_added_list:
                lex_words_list.append(word)

        except Exception as e:
            if e.errno == 10054: #when the primary server crashes it sends an error message to backup server
                msg.insert(tk.END,"[SERVER]:Primary server has crashed.")
                break
...
def connect_to_primary():
    ...
    try:
        conn_thread = threading.Thread(target=receive, args=(CLIENT,)) #New thread is called parallely to actively listen for servers response.
...
def handle_client(conn,addr):
        ...
        elif username=="backup_server":
            backup_servers_list.append(conn)
        else :
                        ...
                        if data == "Disconnect_Client": # if client used disconnect button while loop is broken and client is removed from connected usernames
                            ...
                            for lex in poll_list:
                                if lex not in lex_words_list:# if client given words are not in lexicon then they appended to lexicon data
                                    ...
                                    words_added_list.append(lex.lower())
                            msg.insert(tk.END, f"[RECEIVED LEX WORDS]: {data[4:]}")
                            conn.send("PollingSuccess".encode(FORMAT))

                            if len(backup_servers_list):
                                backup_servers_list[0].send(str(words_added_list).encode(FORMAT))
...
def poll():# is called through polling thread for polling function
    while True:
        time.sleep(5) #Polling is done every 5 seconds.
        for cli in clients:
            cli.sendall("PollingRequest".encode(FORMAT)) # Send a request message to all connected clients


window = tk.Tk()
window.title("BACK UP SERVER GRAPHIC USER INTERFACE")
