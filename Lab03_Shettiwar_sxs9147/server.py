...
def handle_client(conn,addr):
        ...
        if username in usernames :
        ...
        elif username=="backup_server": # backup server connects to primary server here
            backup_servers_list.append(conn)
                    ...
                    try:
                        ...
                        if data == "Disconnect_Client": # if client used disconnect button while loop is broken and client is removed from connected usernames
                            ...
                            for lex in poll_list:
                                if lex not in lex_words_list:# if client given words are not in lexicon then they get appended to lexicon data
                            ...
                            if len(backup_servers_list):
                                lex_upd = json.dumps({"list":words_added_list})
                                backup_servers_list[0].send(lex_upd.encode(FORMAT))#sending the sorted lex words to back up server.
                ...
                active_users.update()#updates the active user list as soon as any changes happen.
...
def connect():
    ...
    while True:
        ...
        thread.setDaemon(True)#using setDaemon helps close the threads as soon as the main thread ends.
        thread.start() # ref: https://origin.geeksforgeeks.org/python-daemon-threads/
    SERVER.close()
