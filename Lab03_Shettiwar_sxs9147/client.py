...
def receive(CLIENT):
    ...
    while True:
        try:
            ...
            if e.errno == 10054:
                msg.insert(tk.END,"Primary Server Crashed.")
                dconclient(CLIENT,disconnect="recon")
                connect(connect="b")
                time.sleep(1)
                continue
            pass
...
def connect(connect="p"):
    ...
    try:
        if connect=="p":
            ...
            CLIENT.connect(B_ADDR)
            msg.insert(tk.END, "Connected to Backup server.")
    except socket.error:
        CLIENT.connect(B_ADDR)
        msg.insert(tk.END, "Connected to Backup server.")
...
def disconnect(disconnect="perm"):
    ...
    if disconnect=="perm":
        ...
        msg.insert(tk.END, "[NOT CONNECTED] You are reconnecting to the backup server.\n")

def dconclient(CLIENT, disconnect="perm"):# this function is executed when client uses the disconnect button
